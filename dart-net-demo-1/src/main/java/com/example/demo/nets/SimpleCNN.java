...
public class SimpleCNN {
	...
	public void run(String basepath) throws Exception {
		...
		if (RunTracker.getRuns().containsKey("SimpleCNN") && RunTracker.getRuns().get("SimpleCNN") == true) {
			return;
		}

		RunTracker.getRuns().put("SimpleCNN", true);
...
//				.setInputType(InputType.convolutional(height, width, channels)).build();
		...
		org.deeplearning4j.zoo.model.SimpleCNN build = org.deeplearning4j.zoo.model.SimpleCNN.builder().seed(rngseed)
				.numClasses(outputNum).inputShape(new int[] { channels, height, width }).build();

		FineTuneConfiguration fineTuneConf = new FineTuneConfiguration.Builder()
				.optimizationAlgo(OptimizationAlgorithm.STOCHASTIC_GRADIENT_DESCENT).updater(new Nesterovs(5e-5))
				.seed(rngseed).build();

		MultiLayerConfiguration mc = build.conf();
		MultiLayerNetwork net = new MultiLayerNetwork(mc);
		net.init();

		OutputLayer outputLayer = new OutputLayer.Builder(LossFunctions.LossFunction.NEGATIVELOGLIKELIHOOD)
				.activation(Activation.SOFTMAX).weightInit(WeightInit.XAVIER).name("Output").nOut(outputNum)
				.build();
		Builder addLayer = new TransferLearning.Builder(net).fineTuneConfiguration(fineTuneConf)
				.addLayer(outputLayer);

		MultiLayerNetwork model = addLayer.build();

//		MultiLayerNetwork model = new MultiLayerNetwork(build.conf());
...
//		.setInputType(InputType.convolutional(height, width, channels)).build();
		...
		ModelSerializer.writeModel(model, new File("SimpleCNN-model.zip"), true);
